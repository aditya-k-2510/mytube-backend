...
const getVideoComments = asyncHandler(async (req, res) => {
    ...
    if(pageNumber<1||pageLimit<1) throw new ApiError(400, "incorrect pagination parameters")  
...
const addComment = asyncHandler(async (req, res) => {
    ...
    if (trimContent.length < 1 || trimContent.length > 100) throw new ApiError(400, "Comment must be 1–100 characters")
...
const deleteComment = asyncHandler(async (req, res) => {
    ...
    const comment = await Comment.findById(commentId)
    if(!comment) throw new ApiError(404, "comment not found")
    if(comment.owner.toString()!==req.user._id.toString()) throw new ApiError(403, "unauthorized request")
